...
function loadPreferences() {
    ...
    prefs.get(
        Constants.Key.interval_mode,
        result => {
            if (isdef(result)) {
                loadIntervalMode(result);
            } else {
                loadIntervalMode(Constants.Defaults.Wallpaper.interval_mode);
            }
        },
        // flag to return even if the result would be 'undefined'
        true
    ...
    // Load the layout mode
    prefs.get(
        Constants.Key.layout_mode,
        result => {
            if (isdef(result)) {
                loadLayoutMode(result);
            } else {
                loadLayoutMode(Constants.Defaults.Wallpaper.layout);
            }
        },
        // flag to return even if the result would be 'undefined'
        true
    ...
    prefs.get(Constants.Key.use_interval, result => {
        if (result === true) {
    ...
    prefs.get(Constants.Key.choose_random, result => {
        if (result === true) {
...
function storeLayoutPreferences() {
    ...
    prefs.set(Constants.Key.layout_mode, layoutMode);
    prefs.apply();
}
